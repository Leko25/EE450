...
#define UDP_PORT "21432"

//Global variable containing respective department filename and extension
// std::string filenames[] = {"DepartmentC.txt", "DepartmentC.txt", "DepartmentC.txt"};
// std::string department_names[] = {"DepartmentC", "DepartmentC", "DepartmentC"};
...
int main(void){
  ...
  printf("<DepartmentC> has TCP port %d and IP address %s for Phase 1\n", (int)ntohs(my_addr.sin_port), s);
  printf("<DepartmentC> is now connected to the admissions office\n");

  //Send packets
  FILE *dept_file;
  char line[15];
  dept_file = fopen("DepartmentC.txt", "r");
  if(dept_file != NULL){
  ...
  printf("<DepartmentC> has UDP port %s and IP address %s for Phase2\n", UDP_PORT, s);
  freeaddrinfo(servinfo);
  ...
  while(numBytes != 8){
    ...
    buff[numBytes] = '\0';
    printf("<Student%c> has been admitted to DepartmentC\n", buff[7]);
    numBytes = recvfrom(sockfd, buff, 99, 0, (struct sockaddr *)&their_addr, &addr_len);
  }
  printf("End of Phase2 for DepartmentC\n");
  close(sockfd);
  return 0;
}
