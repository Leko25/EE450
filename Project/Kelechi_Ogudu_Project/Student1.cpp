...
int main(void){
  ...
  if(stuFile != NULL){
    ...
    printf("<Student1> has received reply from the admissions office\n");
  }else{
  ...
  printf("<Student1> has UDP port %s and IP address %s for Phase2\n", UDP_PORT, s);
  freeaddrinfo(servinfo);
  ...
  buff[numBytes] = '\0';
  printf("<Student1> has received the application result\n");
  printf("End of Phase 2 for <Student1>\n");
  close(sockfd);
  return 0;
}
