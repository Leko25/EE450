...
int main(void){
  ...
  if(stuFile != NULL){
    ...
    printf("<Student5> has received reply from the admissions office\n");
  }else{
  ...
  printf("<Student5> has UDP port %s and IP address %s for Phase2\n", UDP_PORT, s);
  freeaddrinfo(servinfo);
  ...
  buff[numBytes] = '\0';
  printf("<Student5> has received the application result\n");
  printf("End of Phase 2 for <Student5>\n");
  close(sockfd);
  return 0;
}
