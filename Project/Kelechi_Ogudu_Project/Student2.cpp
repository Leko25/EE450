...
int main(void){
  ...
  if(stuFile != NULL){
    ...
    printf("<Student2> has received reply from the admissions office\n");
  }else{
  ...
  printf("<Student2> has UDP port %s and IP address %s for Phase2\n", UDP_PORT, s);
  freeaddrinfo(servinfo);
  ...
  buff[numBytes] = '\0';
  printf("<Student2> has received the application result\n");
  printf("End of Phase 2 for <Student2>\n");
  close(sockfd);
  return 0;
}
