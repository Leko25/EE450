...
#define UDP_PORT "21232"

//Global variable containing respective department filename and extension
// std::string filenames[] = {"DepartmentA.txt", "DepartmentB.txt", "DepartmentC.txt"};
...
int main(void){
  ...
  printf("<DepartmentA> is now connected to the admissions office\n");

  //Send packets
  FILE *dept_file;
  char line[15];
  dept_file = fopen("DepartmentA.txt", "r");
  if(dept_file != NULL){
  ...
  printf("<DepartmentA> has UDP port %s and IP address %s for Phase2\n", UDP_PORT, s);
  freeaddrinfo(servinfo);
  ...
  while(numBytes != 8){
    ...
    buff[numBytes] = '\0';
    printf("<Student%c> has been admitted to DepartmentA\n", buff[7]);
    numBytes = recvfrom(sockfd, buff, 99, 0, (struct sockaddr *)&their_addr, &addr_len);
  }
  printf("End of Phase2 for DepartmentA\n");
  close(sockfd);
  return 0;
}
